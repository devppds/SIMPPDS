...
export default function RiwayatAbsensiPengurusPage() {
    ...
    const columns = [
        { key: 'nama_pengurus', label: 'Nama Pengurus', render: (row) => <div><div style={{ fontWeight: 800 }}>{row.nama_pengurus}</div><small style={{ color: 'var(--text-muted)' }}>{row.jabatan}</small></div> },
        { key: 'divisi', label: 'Divisi', className: 'hide-mobile', render: (row) => <span className="th-badge">{row.divisi}</span> },
